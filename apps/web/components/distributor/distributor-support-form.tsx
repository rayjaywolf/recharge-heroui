...
export function DistributorSupportForm({
  transactions,
  onSubmitted,
}: {
  transactions: DistributorSupportTransactionOption[];
  onSubmitted?: () => void;
}) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch("/api/distributor/disputes", {
        method: "POST",
        body: JSON.stringify({
    ...
    <Card variant="default">
      ...
      <Form onSubmit={handleSubmit}>
        <Card.Content className="space-y-4">
          ...
          <Autocomplete value={selectedTx} onChange={(value) => setSelectedTx(value != null ? String(value) : null)}>
