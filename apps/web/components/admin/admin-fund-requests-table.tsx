...
export type AdminFundRequestRow = {
  id: string;
  retailerName: string;
  amount: number;
  remarks: string | null;
...
export function AdminFundRequestsTable({
  requests,
}: {
  ...
  const handleAction = async (id: string, action: "approve" | "reject") => {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/admin/fund-requests/${id}/${action}`, {
        method: "POST",
      });
  ...
  return (
    <AdminTableCard
      description="Retailers without a distributor can request funds here."
      title={`Direct retailer requests (${requests.length})`}
    >
      {error ? <p className="mb-3 text-sm text-danger">{error}</p> : null}
      {requests.length === 0 ? (
        <AdminTableEmpty message="No pending direct fund requests." />
      ) : (
          ...
          {requests.map((request) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                >
                  {busyId === request.id ? "Working…" : "Approve & credit"}
                </Button>
                <Button
