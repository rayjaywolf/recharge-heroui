...
import {
  updateOperatorBackupProvider,
  updateOperatorProvider,
} from "@/app/(dashboard)/admin/api/actions";
...
function PrimaryApiCard({
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all(
        dirty.map((r) => updateOperatorProvider(r.operator, r.provider))
      );
...
function BackupApisCard({
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all(
        updates.map((u) =>
          updateOperatorBackupProvider(u.operator, u.value, u.slot)
        )
      );
      setSavedRows((current) =>
        current.map((saved) => {
